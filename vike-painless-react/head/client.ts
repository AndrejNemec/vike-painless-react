import { HELMET_ATTRIBUTE, TAG_NAMES, TAG_PROPERTIES } from './constants.js';
import type { Attributes, StateUpdate, TagList } from './types.d.ts';
...
const updateAttributes = (tagName: string, attributes: Attributes) => {
  ...
  const headAttributeString = elementTag.getAttribute(HELMET_ATTRIBUTE);
  const headAttributes = headAttributeString ? headAttributeString.split(',') : [];
  const attributesToRemove = [...headAttributes];
...
let _headCallback: number | null = null;
...
const handleStateChangeOnClient = (newState: StateUpdate) => {
  ...
  if (newState.defer) {
    _headCallback = requestAnimationFrame(() => {
